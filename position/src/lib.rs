...
pub fn get_class_position_ast(
    ...
    match &ast.thing {
        ...
        AstThing::Record(ast_record) => {
            ...
            Ok(vec![PositionSymbol::Range(ast_record.name.range)])
        }
...
pub fn get_method_positions(bytes: &[u8], name: &str) -> Result<Vec<PositionSymbol>, PosionError> {
    ...
    match &ast.thing {
        AstThing::Class(ast_class) => Ok(ast_class
            .block
            .methods
            .iter()
            .map(|i| &i.header.name)
        ...
        AstThing::Interface(ast_interface) => Ok(ast_interface
            ...
            .map(|i| PositionSymbol::Range(i.range))
            .collect()),
        AstThing::Enumeration(ast_enumeration) => Ok(ast_enumeration
            .methods
            .iter()
            .map(|i| &i.header.name)
            .filter(|i| i.value == name)
            .map(|i| PositionSymbol::Range(i.range))
            .collect()),
        AstThing::Annotation(_ast_annotation) => Ok(vec![]),
    }
}
